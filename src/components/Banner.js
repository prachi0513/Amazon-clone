...
function Banner() {
    return (
        <div className="h-banner" style={{marginTop:"3em"}}>
            <div className="h-banner__carousel">
                ...
                >
                    {banners.map((bannerSrc, index) => (
                        <img key={index} alt={`Banner ${index + 1}`} src={bannerSrc} style={{width:"100vw",height:"80vh",marginTop:"65px",border:"none",position:"fixed"}}/>
                    ))}
                </Carousel>
            </div>
        </div>
